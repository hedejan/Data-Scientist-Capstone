# this file contains script to clean and transform datasets
import contextlib
import pickle
...
def clean_RZ(df):
    ...
    df.loc[mask, 'Attribute'] = df.loc[mask,
                                       'Attribute'].str.replace('_RZ', '')
    return df
...
def map_nan(df, attr_unknown):
    ...
    for attribute in attr_unknown.Attribute:
        if attribute in df.columns:
            to_replace = attr_unknown.query(
                'Attribute == @attribute ')['Value'].values[0]
            # clean df
            df[attribute] = df[attribute].replace(
                to_replace=to_replace, value=np.nan)
...
def clean_nan(df, row_thresh, col_thresh):
    ...
    rows_to_retain = nans_per_row_perc <= max(
        row_thresh, upper_whisker(nans_per_row_perc))

    nans_per_col_perc = df.isna().sum(axis=0)/df.shape[0]
    cols_to_retain = nans_per_col_perc <= max(
        col_thresh, upper_whisker(nans_per_col_perc))
...
def classify_features(df, plot=False):
    ...
    # categorical columns
    cat_cols = [c for c in ref.query('Type == "categorical"')[
        'Attribute'].unique() if c in df.columns]

    # ordinal columns
    ord_cols = [c for c in ref.query('Type in ["ordinal","interval"]')[
        'Attribute'].unique() if c in df.columns]
    ord_cols.extend(['DECADE', 'MAINSTREAM', 'WEALTH', 'LIFE_STAGE', ])

    # numeric and skewed columns
    num_cols = [c for c in ref.query('Type == "numeric"')[
        'Attribute'].unique() if c in df.columns]

    # mixed columns
    mixed_cols = [c for c in ref.query('Type == "mixed"')[
        'Attribute'].unique() if c in df.columns]

    # date / datetime columns
    date_cols = [c for c in ref.query('Type == "date"')[
        'Attribute'].unique() if c in df.columns]
...
def feat_eng_data(df, feat_types_dict):
    ...
    with contextlib.suppress(Exception):
        df['WEALTH'] = df['CAMEO_DEUINTL_2015'].replace({'11': 5, '12': 5, '13': 5, '14': 5, '15': 5, '21': 4, '22': 4, '23': 4, '24': 4, '25': 4,
                                                        '31': 3, '32': 3, '33': 3, '34': 3, '35': 3, '41': 2, '42': 2, '43': 2, '44': 2, '45': 2, '51': 1, '52': 1, '53': 1, '54': 1, '55': 1})
        df['LIFE_STAGE'] = df['CAMEO_DEUINTL_2015'].replace({'11': 1, '12': 2, '13': 3, '14': 4, '15': 5, '21': 1, '22': 2, '23': 3, '24': 4, '25': 5,
                                                            '31': 1, '32': 2, '33': 3, '34': 4, '35': 5, '41': 1, '42': 2, '43': 3, '44': 4, '45': 5, '51': 1, '52': 2, '53': 3, '54': 4, '55': 5})
...
def create_column_transformer(df):
    ...
    cat_pipeline = Pipeline([('impute', SimpleImputer(
        missing_values=np.nan, strategy='most_frequent')), ('encode', OneHotEncoder(handle_unknown='ignore'))])
    ord_pipeline = Pipeline([('impute', SimpleImputer(
        missing_values=np.nan, strategy='most_frequent')), ('scale', StandardScaler())])
    num_pipeline = Pipeline([('impute', SimpleImputer(
        missing_values=np.nan, strategy='mean')), ('scale', StandardScaler())])

    transformers = [('numerical', num_pipeline, feat_types_dict['num_cols']), ('ordinal', ord_pipeline, feat_types_dict['ord_cols']), ('binary', bin_pipeline,
                                                                                                                                       feat_types_dict['bin_cols']), ('categorical', cat_pipeline, feat_types_dict['cat_cols']), ('mixed', cat_pipeline, feat_types_dict['mixed_cols'])]
...
def transform_data(df, column_transformer, fit=False):
    ...
    dummy_cat_cols = list(column_transformer.transformers_[
                          3][1].named_steps['encode'].get_feature_names_out(feat_types_dict['cat_cols']))
    dummy_mixed_cols = list(column_transformer.transformers_[
                            4][1].named_steps['encode'].get_feature_names_out(feat_types_dict['mixed_cols']))
...
def match_features(df, pca_in_features):
    ...
    add_features, drop_features = list(
        features_to_match - df_features), list(df_features - features_to_match)
    df[add_features] = 0
